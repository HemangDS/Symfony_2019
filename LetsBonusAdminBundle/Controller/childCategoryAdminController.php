...
class childCategoryAdminController extends CRUDController
{
    /**
     * (non-PHPdoc).
     *
        ...
        if ($this->getRestMethod() == 'POST') {
            ...
            if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                ...
                try {
                    $id = '';
                    $db_parameter_name = '';
                    $slug_response = $this->get('app.category_url_slugger')->urlVerification($id, $object->getUrl(), 'Slug', 'slugName', Constants::CHILD_CATEGORY_IDENTIFIER, $db_parameter_name, $post_parameter_name);
                    ...
                    if ($slug_response == false) {
                        ...
                        $slug = new Slug();
                        $slug->setCategoryType(Constants::CHILD_CATEGORY_IDENTIFIER);
            ...
            if ($isFormValid && (!$this->isInPreviewMode() || $this->isPreviewApproved())) {
                try {
                    ...
                    if ($slug_response == false) {
                        ...
                        $em = $this->getDoctrine()->getManager();
                        $slug = $em->getRepository('iFlairLetsBonusAdminBundle:Slug')->findOneBy(array('categoryType' => Constants::CHILD_CATEGORY_IDENTIFIER, 'categoryId' => $object->getId()));
                        if ($slug) {
                            $slug->setSlugName($slug_response);
                            $em->persist($slug);
                            $em->flush();
    ...
    public function deleteAction($id)
    {
        ...
        if ($this->getRestMethod() == 'DELETE') {
            ...
            try {
                $this->get('app.category_url_slugger')->removeSlug(Constants::CHILD_CATEGORY_IDENTIFIER, $object->getId(), 'Slug', 'categoryType', 'categoryId');
