...
class CmsContactAdminController extends CRUDController
{
    public function createAction()
    {
        if ($this->get('request')->getMethod() == 'POST') {
            $data = $this->get('request');
            $contact_data = $this->getRequest()->request->all();
                    ...
                    if ($key == 'status') {
                        if ($value == 1) {
                            ...
                            if (empty($aboutus_content)) {
                                ...
                                $this->get('request')->getSession()->getFlashBag()->add('error', 'Please Keep One Contact page Enable Only..');
