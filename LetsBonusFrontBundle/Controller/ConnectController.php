...
// src/iFlair/LetsBonusFrontBundle/Controller/ConnectController.php
...
class ConnectController extends ContainerAware
{
     ...
     * @return RedirectResponse
     */
    public function redirectToServiceAction1(Request $request, $service)
    {
        // Check for a specified target path and store it before redirect if present
        $param = $this->container->getParameter('hwi_oauth.target_path_parameter');
        ...
        //echo $baseURL = $this->container->getParameter('base_url');
        //print_r($this->container->getParameter('hwi_oauth.resource_ownermap.configured.secure_area'));
        /*echo $route = $request->get('_route');
        echo $url = $request->getUri();
        echo $providerKey = $this->container->getParameter('hwi_oauth.facebook_front.client_id');
        echo " : ".$this->getFrontAuthorizationUrl($request);
        exit;*/
        /*echo '<pre>';
        print_r($request);exit;*/
        //return new RedirectResponse($urlf);
        if ($request->attributes->get('custom') == 'secure') {
            if ($request->attributes->get('service') == 'facebook') {
                return new RedirectResponse('https://www.facebook.com/v2.0/dialog/oauth?response_type=code&client_id=1282743601770979&scope=email++public_profile&redirect_uri=http%3A%2F%2Flocalhost%3A8000%2Fsecure%2Flogin%2Fcheck-facebook&display=page');
            } elseif ($request->attributes->get('service') == 'google') {
                return new RedirectResponse('https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=771712207036-9c3du1r73b2ntj1j43d3ss7vt6vl4poo.apps.googleusercontent.com&scope=email+profile&redirect_uri=http%3A%2F%2Flocalhost%3A8000%2Fsecure%2Flogin%2Fcheck-google');
            }
        }
        /*return new RedirectResponse($this->getFrontAuthorizationUrl($request));
        return new RedirectResponse($this->container->get('hwi_oauth.security.oauth_utils')->getAuthorizationUrl($request, $service));*/
    }

    public function getFrontAuthorizationUrl(Request $request)
    {
        $url = 'https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=771712207036-9c3du1r73b2ntj1j43d3ss7vt6vl4poo.apps.googleusercontent.com&scope=email+profile&redirect_uri=http%3A%2F%2Flocalhost%3A8000%2Fsecure%2Flogin%2Fcheck-google';
        $urlf = 'https://www.facebook.com/v2.0/dialog/oauth?response_type=code&client_id=1282743601770979&scope=email++public_profile&redirect_uri=http%3A%2F%2Flocalhost%3A8000%2Fsecure%2Flogin%2Fcheck-facebook&display=page';
        $uri = '';

        if ($request->attributes->get('custom') == 'secure') {
            $base = explode($request->attributes->get('custom'), $request->getUri());
            if ($request->attributes->get('service') == 'facebook') {
                $uri .= 'https://www.facebook.com/v2.0/dialog/oauth?response_type=code';
                $uri .= '&client_id='.$this->container->getParameter('hwi_oauth.facebook_front.client_id');
                $uri .= '&scope='.str_replace(', ', '++', $this->container->getParameter('hwi_oauth.facebook_front.scope'));
                $uri .= '&redirect_uri='.urlencode($base[0].'secure_area/login/check-facebook');
                $uri .= '&display=page';
            } elseif ($request->attributes->get('service') == 'google') {
                $uri .= 'https://accounts.google.com/o/oauth2/auth?response_type=code';
                $uri .= '&client_id='.$this->container->getParameter('hwi_oauth.google_front.client_id');
                $uri .= '&scope='.str_replace(' ', '+', $this->container->getParameter('hwi_oauth.google_front.scope'));
                $uri .= '&redirect_uri='.urlencode($base[0].'secure_area/login/check-google');
            }
        }
        ...
        //echo $route = $request->get('_route');
        /*echo '<pre>';
        print_r($request->attributes->get('custom'));
        print_r($request->attributes->get('service'));
        exit;
        return $this->container->getParameter('hwi_oauth.facebook_front.client_id');*/
    }
